...
public:
    ...
    static void load(GridSystem& gridSystem, const std::string& filename) {
        ...
        std::string id1, tex1; int cnt1;
        std::string id2, tex2; int cnt2;
        std::string id3, tex3; int cnt3; 
        
        while (file >> x >> y >> type >> texName >> rotation >> progress >> fuel 
                    >> id1 >> tex1 >> cnt1 
                    >> id2 >> tex2 >> cnt2 
                    >> id3 >> tex3 >> cnt3) {
            ...
            if (x < gridSystem.gridMatrix.size() && y < gridSystem.gridMatrix[0].size()) {
                ...
                if (id1 != "empty_slot") loadedBlock.inputInventory.addItem(Item(id1, tex1), cnt1);
                if (id2 != "empty_slot") loadedBlock.outputInventory.addItem(Item(id2, tex2), cnt2);
                if (id3 != "empty_slot") loadedBlock.fuelInventory.addItem(Item(id3, tex3), cnt3);
