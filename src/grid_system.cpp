...
void GridSystem::placeBlock(GameBlock &block, unsigned gridX, unsigned gridY)
{
    if (gridX < gridMatrix.size() && gridY < gridMatrix[0].size())
    {
        ...
        if (!newBlock.textureName.empty())
        {
            ...
            if (tex)
                newBlock.blockShape.setTexture(tex);
        }
        ...
        for (auto &block : row)
        {
            if (!block.textureName.empty())
            {
                if (block.blockShape.getTexture() == nullptr)
                {
                    ...
                    if (tex)
                        block.blockShape.setTexture(tex);
                }
                ...
                if (block.blockType == "furnace")
                {
                    if (block.inputInventory.itemCount > 0)
                        drawItem(block.inputInventory.item, -8.f, -8.f, 0.4f);
                    if (block.outputInventory.itemCount > 0)
                        drawItem(block.outputInventory.item, 8.f, -8.f, 0.4f);
                    if (block.fuelInventory.itemCount > 0)
                        drawItem(block.fuelInventory.item, 0.f, 8.f, 0.4f);
                }
                else if (block.blockType == "hopper")
                {
                    if (block.inputInventory.itemCount > 0)
                        drawItem(block.inputInventory.item, 0.f, 0.f, 0.5f);
                }
                else
                {
                    const Item *itemToShow = nullptr;
                    if (block.outputInventory.itemCount > 0)
                        itemToShow = &block.outputInventory.item;
                    else if (block.inputInventory.itemCount > 0)
                        itemToShow = &block.inputInventory.item;
                    if (itemToShow)
                        drawItem(*itemToShow, 6.f, 6.f, 0.5f);
                }
            }
        }
    }
        ...
        for (int y = 0; y < gridMatrix[x].size(); ++y)
        {
            ...
            if (block.blockType == "pickaxe")
            {
                ...
                if (block.rotationState == 0)
                    dy = 1;
                else if (block.rotationState == 1)
                    dx = -1;
                else if (block.rotationState == 2)
                    dy = -1;
                else if (block.rotationState == 3)
                    dx = 1;
                int tx = x + dx;
                int ty = y + dy;
                if (tx >= 0 && tx < gridMatrix.size() && ty >= 0 && ty < gridMatrix[0].size())
                {
                    ...
                    bool mineAction = false;
                    std::string itemId = "", itemTex = "";
                    if (target.blockType == "iron_ore")
                    {
                        if (currentTick % 5 == 0)
                        {
                            itemId = "raw_iron";
                            itemTex = "/Sprites/item/raw_iron.png";
                            mineAction = true;
                        }
                    }
                    else if (target.blockType == "coal_ore")
                    {
                        if (currentTick % 2 == 0)
                        {
                            itemId = "coal";
                            itemTex = "/Sprites/item/coal.png";
                            mineAction = true;
                        }
                    }
                    if (mineAction && !itemId.empty() && block.outputInventory.itemCount < 64)
                        block.outputInventory.addItem(Item(itemId, itemTex), 1);
                }
            }
            ...
            if (block.blockType == "furnace")
            {
                ...
                if (hasInput && hasFuel)
                {
                    ...
                    for (const auto &r : furnaceRecipes)
                    {
                        if (r.inputId == block.inputInventory.item.itemId)
                        {
                            currentRecipe = &r;
                            break;
                        }
                    ...
                    if (currentRecipe)
                    {
                        if (block.fuelLevel <= 0 && block.fuelInventory.itemCount > 0)
                        {
                            block.fuelInventory.removeItem(1);
                            block.fuelLevel = 200.f;
                        }
                        if (block.fuelLevel > 0)
                        {
                            block.fuelLevel -= 1.0f;
                            block.craftingProgress += 1;
                            if (block.craftingProgress >= currentRecipe->durationTicks)
                            {
                                Item resultItem(currentRecipe->outputId, currentRecipe->outputTexture);
                                if (block.outputInventory.addItem(resultItem, 1))
                                {
                                    block.inputInventory.removeItem(1);
                                    block.craftingProgress = 0;
                                }
                                else
                                {
                                    block.craftingProgress = currentRecipe->durationTicks;
                                }
                            }
                        }
                        else
                        {
                            if (block.craftingProgress > 0)
                    ...
                    if (block.inputInventory.itemCount > 0)
                    {
                        ...
                        if (frontX >= 0 && frontX < gridMatrix.size() && frontY >= 0 && frontY < gridMatrix[0].size())
                        {
                            ...
                            if (isValidContainer)
                            {
                                ...
                                {
                                    if (toMove.itemId == "coal")
                                    {
                                        if (target.fuelInventory.addItem(toMove, 1))
                                            success = true;
                                    }
                                    else
                                    {
                                        if (target.inputInventory.addItem(toMove, 1))
                                            success = true;
                                    }
                                }
                                else if (target.blockType == "target")
                                {
                                    if (taskManager.isNeeded(toMove.itemId))
                                    {
                                        if (target.inputInventory.addItem(toMove, 1))
                                            success = true;
                                    }
                        ...
                        if (backX >= 0 && backX < gridMatrix.size() && backY >= 0 && backY < gridMatrix[0].size())
                        {
                            ...
                            if (source.blockType == "furnace" || source.blockType == "pickaxe")
                            {
                                if (source.outputInventory.itemCount > 0)
                                {
                                    ...
                                    if (block.inputInventory.addItem(toMove, 1))
                                        source.outputInventory.removeItem(1);
                                }
                            }
                            else if (source.blockType == "hopper")
                            {
                                if (source.inputInventory.itemCount > 0)
                                {
                                    ...
                                    if (block.inputInventory.addItem(toMove, 1))
                                        source.inputInventory.removeItem(1);
                                }
                            }
                        }
                    }
