...
const CraftingRecipe* findRecipe(const std::string& inputId) {
    for (const auto& r : RECIPES) {
        if (r.inputItemId == inputId) return &r;
    }
    return nullptr;
}
...
void calculateDirection(int rotationState, int& offsetX, int& offsetY) {
    offsetX = 0; offsetY = 0;
    if (rotationState == 0) offsetY = -1;      
    else if (rotationState == 1) offsetX = 1;  
    else if (rotationState == 2) offsetY = 1;  
    else if (rotationState == 3) offsetX = -1; 
}
...
void FurnaceLogic::update(int gridX, int gridY, GridSystem& gridSystem, unsigned long long currentTick, TaskManager& taskManager) {
    ...
    if (furnace.inputInventory.itemCount > 0) {
        ...
        if (recipe) {
            ...
            if (furnace.outputInventory.itemCount == 0) canOutput = true;
            else if (furnace.outputInventory.item == resultItem && furnace.outputInventory.itemCount < 64) canOutput = true;
            ...
            } else furnace.craftingProgress = 0;
        } else furnace.craftingProgress = 0;
    } else furnace.craftingProgress = 0;
}
...
void HopperLogic::update(int gridX, int gridY, GridSystem& gridSystem, unsigned long long currentTick, TaskManager& taskManager) {
    ...
    int inputOffsetX, inputOffsetY; calculateDirection(rot, inputOffsetX, inputOffsetY); 
    int outputOffsetX, outputOffsetY; calculateDirection((rot + 2) % 4, outputOffsetX, outputOffsetY); 

    int inputX = gridX + inputOffsetX; int inputY = gridY + inputOffsetY;
    int outputX = gridX + outputOffsetX; int outputY = gridY + outputOffsetY;
    ...
    if (gridSystem.gridMatrix[gridX][gridY].inputInventory.itemCount < 64) {
        if (inputX >= 0 && inputX < (int)gridSystem.gridMatrix.size() && inputY >= 0 && inputY < (int)gridSystem.gridMatrix[0].size()) {
            ...
            if (sourceBlock.blockType == "furnace" || sourceBlock.blockType == "crafter") targetSourceInv = &sourceBlock.outputInventory; 
            else targetSourceInv = &sourceBlock.inputInventory;
...
void PickaxeLogic::update(int gridX, int gridY, GridSystem& gridSystem, unsigned long long currentTick, TaskManager& taskManager) {
    ...
    int rot = gridSystem.gridMatrix[gridX][gridY].rotationState;
    int offsetX, offsetY; calculateDirection((rot + 1) % 4, offsetX, offsetY);
    int targetX = gridX + offsetX; int targetY = gridY + offsetY;
    ...
    if (targetX >= 0 && targetX < (int)gridSystem.gridMatrix.size() && targetY >= 0 && targetY < (int)gridSystem.gridMatrix[0].size()) {
        ...
        if (targetBlock.blockType == "iron_ore") {
            // Добываем raw_iron (сырое железо)
...
BlockLogic* LogicFactory::createLogic(const std::string& type) {
    if (type == "target") return new TargetLogic();
    if (type == "furnace") return new FurnaceLogic();
    if (type == "hopper")  return new HopperLogic();
    if (type == "pickaxe") return new PickaxeLogic();
